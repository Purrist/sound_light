...
FRONTEND_FOLDER = os.path.join(os.path.dirname(BACKEND_FOLDER), 'frontend')
STATIC_FOLDER = os.path.join(BACKEND_FOLDER, 'static')
...
def initialize_defaults():
    ...
    if not os.path.exists(default_soundset_path):
        ...
        print("Created default soundset: 海洋.json")
    ...
    if not os.path.exists(default_controlset_path):
        default_controlset_data = {
            "breathsPerMin": "6",
            "masterKelvinStart": "2000",
            "masterHexStart": "#f57e0f",
            "masterKelvinEnd": "8000",
            "masterHexEnd": "#8cb1ff",
            "kelvinSliderDefault": "5000",
            "kelvinDefault": "5000",
            "kelvinSliderMin": "7000",
            "kelvinMin": "7000",
            "kelvinSliderMax": "3000",
            "kelvinMax": "3000",
            "defaultColor": "#c19887",
            "warmColor": "#9ea9d7",
            "coolColor": "#e48737",
        ...
        print("Created default controlset: 默认配置.json")
        if not os.path.exists(DEFAULT_CONFIG_FILE):
            with open(DEFAULT_CONFIG_FILE, 'w', encoding='utf-8') as f: f.write('默认配置')
            print("Set '默认配置' as default.")

@app.route('/')
def serve_index(): return send_from_directory(FRONTEND_FOLDER, 'index.html')
@app.route('/<path:filename>')
def serve_frontend_files(filename):
    if filename.startswith('api/') or filename.startswith('static/'): return "Not Found", 404
    return send_from_directory(FRONTEND_FOLDER, filename)
@app.route('/static/<path:folder>/<path:filename>')
def serve_static_audio(folder, filename): return send_from_directory(os.path.join(STATIC_FOLDER, folder), filename)
...
def get_soundset_by_name(name):
    ...
    if os.path.exists(file_path): return send_from_directory(SOUNDSET_FOLDER, f"{name}.json")
...
def get_controlset_by_name(name):
    ...
    if os.path.exists(file_path): return send_from_directory(CONTROLSET_FOLDER, f"{name}.json")
